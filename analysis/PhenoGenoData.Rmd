...
A matrix de marcadores do arquivo `DadosGBSCaroline.rds` possui 1,446 clones e 72,023 marcadores SNPs em formato de haplótipos (A, T, C, G, K, M, R, S, Y, W), temos que transformar para dose alélica.
...
GBSDataCar2 <- GBSDataCar[rownames(GBSDataCar) %in% AllDataInfo$Acessos, ]

AllDataInfo2 <- AllDataInfo[AllDataInfo$Acessos %in% rownames(GBSDataCar2),]
AllDataInfo2 <- AllDataInfo2[order(AllDataInfo2$Acessos),] 
rownames(AllDataInfo2) <- AllDataInfo2$Acessos
AllDataInfo2$Acessos <- NULL

saveRDS(AllDataInfo2, file = here::here("output", "DataSelPreparedCar.rds"))
```

## Controle de qualidade de Marcadores SNPs - MAF

```{r MAF, eval = F}
Freq <- colMeans(GBSDataCar2, na.rm = T)/2
...
GBSDataCar2 <- as.data.frame(GBSDataCar2[order(rownames(GBSDataCar2)),(MAF >= sqrt(1/nrow(GBSDataCar2)))])

saveRDS(GBSDataCar2, file = here::here("data", "DadosGBSCaroline2.rds"))
```

Next - [Coleções Nucleares](CC.html)
