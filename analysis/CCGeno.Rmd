...
CCSelAM.DARwin <- read.table(here::here("output", "CarDarwinMatrix.don"),
                             sep = "\t", header = T,skip = 2,
                             nrows = 1486)
table(CCSelAM.DARwin$Tree.Sample_166)
ClonesSel <- CCSelAM.DARwin %>% filter(Tree.Sample_166 == "Kept") %>%
  select(N)
...
CCSelMR.DARwin <- read.table(here::here("output", "MatrizMDCaroline.DON"),
                             sep = "\t", header = T,skip = 2,
                             nrows = 1486)
table(CCSelMR.DARwin$Tree.Sample_166)
ClonesSel <- CCSelMR.DARwin %>% filter(Tree.Sample_166 == "Kept") %>%
  select(Name)
...
CCSelCE.DARwin <- read.table(here::here("output","MatrizCECaroline.DON"),
                             sep = "\t", header = T, skip = 2,
                             nrows = 1486)
table(CCSelCE.DARwin$Tree.Sample_166)
ClonesSel <- CCSelCE.DARwin %>% filter(Tree.Sample_191 == "Kept") %>%
  select(Name)
