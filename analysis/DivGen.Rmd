...
AlldataSetQuant %>% group_by(Data, Method) %>%
...
### **Table 3.** Means and variances for Nuclear Collections of quantitative traits 
...
#### **Table 4.** Kappa Index for coincidence of the clone selection for the 12 nuclear collections.
...
sink(file = here::here("output", "ClonesSelCCPhen.txt"))
VennDPNC
sink()
...
sink(file = here::here("output", "ClonesSelCCGenCoreHunter.txt"))
VennDGNC1
sink()
...
sink(file = here::here("output", "ClonesSelCCGenDARwin.txt"))
VennDGNC2
sink()
```
...
write.table(SHRelPhen, file = here::here("output", "GanhoRelativoShannonWeaverPhen.csv"),
            quote = F, sep = ",")
...
#### **Table 5.** Mean improvement of the Shannon-Weaver diversity index in % of nuclear core collections estimated of Phenotypic data compared to the Bank of Germplasm
...
write.table(SHRelGen, file = here::here("output", "GanhoRelativoShannonWeaverGen.csv"),
            quote = F, sep = ",")
...
#### **Table 6.** Mean improvement of the Shannon-Weaver diversity index in % of nuclear core collections estimated of Genotypic data compared to the Bank of Germplasm
...
#### **Table 7.** Formato da entrado do objeto para realizar o boxplot
...
#### **Table 8.** Formato da entrado do objeto para realizar o barplot
...
#### **Table 9.** Shannon-Weaver indexes for Cassava quantitative traits of the Bank of germplasm and 3 nuclear core collections.
...
#### **Table 10.** Shannon-Weaver indexes for Cassava quality traits of the Bank of germplasm and 3 nuclear core collections.
...
write.table(SHRelFinal, file = here::here("output", "GanhoRelativoShannonWeaverFinal.csv"),
            quote = F, sep = ",")
...
#### **Table 11.** Mean improvement of the Shannon-Weaver diversity index in % of nuclear core collections estimated of Phenotypic data compared to the Bank of Germplasm
...
```{r Mean and Variance of Final Nuclear Collections}
AlldataSetQuant$Method <- factor(AlldataSetQuant$Method,
                                 levels = c("BAG", "Pheno", "CoreHunter", "DARWin", "NCSel"))
AlldataSetQuant %>% group_by(Method) %>%
  summarise(ComprFilotaxiaMean = mean(ComprFilotaxia, na.rm = T),
            ComprFilotaxiaVar = var(ComprFilotaxia, na.rm = T),
            ComprLobuloMean = mean(ComprLobulo, na.rm = T),
            ComprLobuloVar = var(ComprLobulo, na.rm = T),
            ...
            APMean = mean(AP, na.rm = T),
            APVar = var(AP, na.rm = T)) -> NCMeanVar
write.table(NCMeanVar, file = here::here("output", "MeansVarNCFinal.csv"),
            quote = F, row.names = F, sep = ",")
```

### **Table 12.** Means and variances for Nuclear Collections of quantitative traits 
...
### Final Venn diagram for each Nuclear Collection Group

```{r}

SelCC <- tibble(Clones = BAGClones)
CCFMet <- unique(Alldataset$Method)
...
library(ggvenn)
VennFinal <- list(
  Pheno = rownames(SelCC)[SelCC$Pheno == 1],
  CoreHunter = rownames(SelCC)[SelCC$CoreHunter == 1],
  DARWin = rownames(SelCC)[SelCC$DARWin == 1])
VDFinal <- ggvenn(VennFinal, fill_color = c("red", "blue", "green"),
              fill_alpha = 0.6, set_name_size = 5)

sink(file = here::here("output", "ClonesSelCCFinal.txt"))
VennFinal
sink()
```
...
#### Fig 8. Final Venn diagram for the three groups of Nuclear Collections
```{r Fig 8, fig.height = 12, echo = FALSE}
VDFinal
```
